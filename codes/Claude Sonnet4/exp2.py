...
class PuMVRBlindClaudeEvaluator:
    """
    Evaluator class for Experiment 2 (Blind Baseline) with Vertex AI / Claude.
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_exp2_claude_blind_{safe_model_name}"
    ...
    def _authenticate(self):
        ...
        print(f"   • Region : {self.region} (Critical for Claude)")
        try:
    ...
    def _get_csv_path(self, script: str) -> str:
        ...
        filename = f"res_exp2_claude_blind_{safe_model_name}_{script}.csv"
    ...
    def _create_prompt(self, question: str, options: List[str], script: str) -> str:
        ...
        prompt = (
            f"Question: {question}\n"
            f"Options:\n{formatted_options}\n\n"
            f"Constraint: {script_instruction[script]} Copy the exact text of the correct option.\n"
            f"CRITICAL RULES:\n"
    ...
    def _generate_answer(self, question: str, options: List[str], script: str) -> tuple[str, str]:
        """
        Generate answer using Claude API - WITHOUT IMAGE INPUT
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Experiment 2: Blind Baseline (Vertex AI / Claude) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRBlindClaudeEvaluator(
        project_id=GCP_PROJECT_ID,
        region=GCP_REGION,
        model_id=MODEL_NAME,
        batch_size=None,
        output_dir="./results_exp2_claude_blind"
    )
    evaluator.run()
