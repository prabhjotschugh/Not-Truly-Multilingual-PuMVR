...
class PuMVRFewShotGrokEvaluator:
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_exp3_grok_fewshot_{safe_model_name}"
    ...
    def _authenticate(self):
        ...
        if not self.api_key or "YOUR_XAI_API_KEY" in self.api_key:
            print(" WARNING: API Key appears invalid. Please set XAI_API_KEY.")
        else:
    ...
    def _get_csv_path(self, source_tag: str, target_script: str) -> str:
        ...
        filename = f"res_exp3_grok_{safe_model_name}_{source_tag}_to_{target_script}.csv"
    ...
    def _encode_image_to_base64(self, image: Image.Image) -> str:
        """Convert PIL Image to base64 string for Grok API"""
        if image.mode != "RGB":
    ...
    def _construct_grok_inputs(self, shots: List[Dict], target_row: Dict, target_script: str):
        """
        Builds the Message Payload for Grok.
        ...
        for shot in shots:
            shot_b64 = self._encode_image_to_base64(shot['image'])
        ...
        target_b64 = self._encode_image_to_base64(target_row['image'])
        messages.append({
            "role": "user",
            "content": [
                {
    ...
    def _generate_answer(self, messages: List[Dict]) -> tuple[str, str]:
        """
        Generate answer using Grok API via OpenAI SDK.
        """
        try:
    ...
    def _extract_answer(self, raw_response: str, options: List[str]) -> str:
        """
        Post-process extraction (Identical to original script)
        """
    ...
    def _evaluate_condition(self, source_tag: str, target_script: str):
        ...
        for row in dataset_subset:
            ...
            try:
                ...
                messages = self._construct_grok_inputs(shots, row, target_script)
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRFewShotGrokEvaluator(
        model_id=model_name,
        k_shots=3,
        batch_size=5,
        output_dir="./results_exp3_grok_fewshot"
    )
