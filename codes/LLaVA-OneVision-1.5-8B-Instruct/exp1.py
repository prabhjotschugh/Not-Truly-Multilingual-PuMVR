...
class PuMVRLLaVAEvaluator:
    """
    Evaluator class for LLaVA-OneVision-1.5-8B-Instruct on PuMVR dataset.
    """
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_llava_{safe_model_name}"
    ...
    def _load_model(self):
        """Load LLaVA-OneVision Model (Using AutoModel to support rice_vit)"""
    ...
    def _get_csv_path(self, script: str) -> str:
        filename = f"res_llava_{script}.csv"
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        for script in self.SCRIPTS:
            ...
            if os.path.exists(filepath):
                ...
                accuracies[script], counts[script], all_dfs[script] = acc, len(df), df
            else:
                accuracies[script], counts[script], all_dfs[script] = 0.0, 0, pd.DataFrame()
        scr = 0.0
        if not all_dfs[self.SCRIPTS[0]].empty:
            ...
            for script in self.SCRIPTS[1:]:
                if not all_dfs[script].empty:
                    ...
                    common_ids = set(); break
            if common_ids:
                consistent_count = sum(1 for uid in common_ids if all(all_dfs[s][all_dfs[s]['id'] == uid].iloc[0]['is_correct'] for s in self.SCRIPTS))
                scr = consistent_count / len(common_ids)
        return {"accuracies": accuracies, "scr": scr, "counts": counts}

    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Evaluator (LLaVA-OneVision) | Model: {self.model_id}")
        print("=" * 60)
        ...
        self.summary = {"model": self.model_id, "metrics": metrics, "duration": duration, "timestamp": datetime.now().isoformat()}
        print("\n" + "="*60 + "\nRESULTS SUMMARY\n" + "="*60)
        ...
        with open(summary_path, "w") as f: json.dump(self.summary, f, indent=2)
        print(f" Summary saved to {summary_path}")
        return self.summary

if __name__ == "__main__":
    model_name = "lmms-lab/LLaVA-OneVision-1.5-8B-Instruct"
    evaluator = PuMVRLLaVAEvaluator(
        model_id=model_name,
        batch_size=375,
        output_dir="./results_llava_test"
    )
    evaluator.run()
