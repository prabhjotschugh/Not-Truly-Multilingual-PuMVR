...
class PuMVRBlindLLaVAEvaluator:
    """
    Evaluator class for Experiment 2 (Blind Baseline) with LLaVA-OneVision.
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_exp2_llava_blind_{safe_model_name}"
    ...
    def _get_csv_path(self, script: str) -> str:
        ...
        filename = f"res_exp2_llava_blind_{safe_model_name}_{script}.csv"
    ...
    def _generate_answer(self, question: str, options: List[str], script: str) -> tuple[str, str]:
        """
        Generate answer using LLaVA-OneVision logic - WITHOUT IMAGE INPUT
        """
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Experiment 2: Blind Baseline (LLaVA-OneVision) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRBlindLLaVAEvaluator(
        model_id=model_name,
        batch_size=375,
        output_dir="./results_exp2_llava_blind"
    )
    evaluator.run()
