...
class PuMVRLlamaEvaluator:
    """
    Evaluator class for Llama-3.2-11B-Vision models on PuMVR dataset.
    """
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_llama_{safe_model_name}"
    ...
    def _get_csv_path(self, script: str) -> str:
        filename = f"res_llama_{script}.csv"
    ...
    def _generate_answer(self, question: str, options: List[str], script: str, image: Image.Image) -> tuple[str, str]:
        """
        Generate answer using Llama-3.2 Vision logic.
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Evaluator (Llama-3.2) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRLlamaEvaluator(
        model_id=model_name,
        batch_size=375,
        output_dir="./results_llama_exp1"
    )
    evaluator.run()
