...
class PuMVRBlindLlamaEvaluator:
    """
    Evaluator class for Experiment 2 (Blind Baseline) with Llama-3.2-11B-Vision.
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_exp2_llama_blind_{safe_model_name}"
    ...
    def _load_model(self):
        """Load Llama-3.2 Vision Model and Processor (Blind Mode)"""
    ...
    def _get_csv_path(self, script: str) -> str:
        ...
        filename = f"res_exp2_llama_blind_{safe_model_name}_{script}.csv"
    ...
    def _generate_answer(self, question: str, options: List[str], script: str) -> tuple[str, str]:
        """
        Generate answer using Llama-3.2 logic - WITHOUT IMAGE INPUT
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Experiment 2: Blind Baseline (Llama-3.2) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRBlindLlamaEvaluator(
        model_id=model_name,
        batch_size=375,
        output_dir="./results_exp2_llama_blind"
    )
    evaluator.run()
