...
class PuMVROpenAIEvaluator:
    """
    Evaluator class for OpenAI models on PuMVR dataset.
    """
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_openai_{safe_model_name}"
    ...
    def _authenticate(self):
        ...
        if not self.api_key or "sk-" not in self.api_key:
             print(" WARNING: OPENAI_API_KEY appears invalid. Please hardcode your key at the top.")
        else:
    ...
    def _get_csv_path(self, script: str) -> str:
        filename = f"res_openai_{script}.csv"
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Evaluator (OpenAI API) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    model_name = "gpt-4o"
    evaluator = PuMVROpenAIEvaluator(
        model_id=model_name,
        batch_size=5,
        output_dir="./results_openai_test"
    )
    evaluator.run()
