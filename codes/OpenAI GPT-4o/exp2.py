...
class PuMVRBlindOpenAIEvaluator:
    """
    Evaluator class for Experiment 2 (Blind Baseline) with OpenAI API.
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_exp2_openai_blind_{safe_model_name}"
    ...
    def _authenticate(self):
        ...
        if not self.api_key or "sk-" not in self.api_key:
            print(" WARNING: OPENAI_API_KEY appears invalid or missing. Please hardcode your key at the top.")
        else:
            print(f" Authenticating with OpenAI API...")
        self.client = OpenAI(api_key=self.api_key)
        print(f"  Configuring Model: {self.model_id} (BLIND MODE - TEXT ONLY)")
    ...
    def _get_csv_path(self, script: str) -> str:
        ...
        filename = f"res_exp2_openai_blind_{safe_model_name}_{script}.csv"
    ...
    def _create_prompt(self, question: str, options: List[str], script: str) -> str:
        """Create prompt for blind evaluation - EXACTLY AS REQUESTED"""
    ...
    def _generate_answer(self, question: str, options: List[str], script: str) -> tuple[str, str]:
        """
        Generate answer using OpenAI API - WITHOUT IMAGE INPUT
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Experiment 2: Blind Baseline (OpenAI API) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    model_name = "gpt-4o"
    evaluator = PuMVRBlindOpenAIEvaluator(
        model_id=model_name,
        batch_size=None,
        output_dir="./results_exp2_openai_blind"
    )
    evaluator.run()
