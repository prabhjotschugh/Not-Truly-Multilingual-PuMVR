...
class PuMVRQwenEvaluator:
    """
    Evaluator class for Qwen2-VL models on PuMVR dataset.
    """
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_qwen_{safe_model_name}"
    ...
    def _get_csv_path(self, script: str) -> str:
        filename = f"res_qwen_{script}.csv"
    ...
    def _generate_answer(self, question: str, options: List[str], script: str, image: Image.Image) -> tuple[str, str]:
        """
        Generate answer using Qwen2-VL logic.
    ...
    def run(self) -> Dict[str, Any]:
        print("=" * 60)
        print(f"PuMVR Evaluator (Qwen2-VL) | Model: {self.model_id}")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRQwenEvaluator(
        model_id=model_name,
        batch_size=375,
        output_dir="./results_qwen_7B_instruct_exp1"
    )
    evaluator.run()
