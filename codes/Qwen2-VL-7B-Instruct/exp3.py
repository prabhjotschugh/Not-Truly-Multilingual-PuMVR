...
class PuMVRFewShotEvaluator:
    ...
    def __init__(
        ...
        self.output_dir = output_dir or f"./results_exp3_qwen_fewshot_{safe_model_name}"
    ...
    def _authenticate(self):
        """Log in to Hugging Face Hub - SAME AS BEFORE"""
    ...
    def _set_seed(self):
        """Set random seed for reproducibility - SAME AS BEFORE"""
    ...
    def _load_model(self):
        """Load Qwen2-VL Model and Processor - SAME AS BEFORE"""
    ...
    def _load_dataset(self):
        """Load PuMVR dataset - SAME AS BEFORE"""
    ...
    def _get_csv_path(self, source_tag: str, target_script: str) -> str:
        """Get CSV file path - Modified for Experiment 3 naming"""
        safe_model_name = self.model_id.replace(":", "_").replace("/", "_")
        filename = f"res_exp3_qwen_{safe_model_name}_{source_tag}_to_{target_script}.csv"
    ...
    def _save_single_result(self, result: Dict, source_tag: str, target_script: str):
        """
        CRITICAL: Writes to CSV immediately after processing. - SAME AS BEFORE
        """
        filepath = self._get_csv_path(source_tag, target_script)
    ...
    def _construct_conversation(self, shots: List[Dict], target_row: Dict, target_script: str) -> List[Dict]:
        """
        Builds the Qwen2-VL conversation format with few-shot examples.
        CRITICAL: The 'assistant' messages now contain ONLY the answer text.
        """
        script_instruction = {
            "gurmukhi": "Gurmukhi (ਗੁਰਮੁਖੀ)",
            "shahmukhi": "Shahmukhi (شاہ مکھی)",
    ...
    def _generate_answer(self, conversation: List[Dict]) -> tuple[str, str]:
        """
        Generate answer using Qwen2-VL logic with few-shot conversation.
    ...
    def _extract_answer(self, raw_response: str, options: List[str]) -> str:
        """
        Post-process extraction - SIMILAR TO YOUR CODE
        """
...
if __name__ == "__main__":
    ...
    evaluator = PuMVRFewShotEvaluator(
        model_id=model_name,
        k_shots=3,
        batch_size=375,
        output_dir="./results_exp3_qwen_fewshot"
    )
